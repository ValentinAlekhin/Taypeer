//! Validated, nonsensitive appearance file format.
mod options;
...
/// Serializable appearance choices; geometry is stored at the 16-pixel base scale.
#[non_exhaustive]
pub struct Preferences {
    /// Interface language.
    pub language: Language,
    /// Explicit or system-following theme choice.
    pub theme: ThemePreference,
    /// Pixel anchor for the application rem scale.
    pub font_size: u8,
    /// Group panel width at the default base scale.
    pub group_width: f32,
    /// Legacy entry panel width retained for file compatibility.
    pub entry_width: f32,
    #[serde(default = "default_inspector_width")]
    /// Inspector panel width at the default base scale.
...
impl Preferences {
    /// Read validated preferences; an absent file uses defaults, an invalid file fails.
    pub fn load_from(path: &Path) -> Result<Self, PreferencesError> {
        let text = match std::fs::read_to_string(path) {
            Ok(text) => text,
            Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
                return Ok(Self::default());
            }
            Err(_) => return Err(PreferencesError::Unavailable),
        };
        let prefs: Self = toml::from_str(&text).map_err(|_| PreferencesError::Invalid)?;
        prefs.validate()?;
        Ok(prefs)
    }
    ...
    /// Replace a valid preferences file atomically; never overwrite an unknown schema.
    pub fn save_to(&self, path: &Path) -> Result<(), PreferencesError> {
        ...
        Self::load_from(path)?;
        std::fs::create_dir_all(path.parent().ok_or(PreferencesError::Unavailable)?)
            .map_err(|_| PreferencesError::Unavailable)?;
        let data = toml::to_string(self).map_err(|_| PreferencesError::Unavailable)?;
        let temporary = path.with_extension("toml.new");
        std::fs::write(&temporary, data).map_err(|_| PreferencesError::Unavailable)?;
        std::fs::rename(temporary, path).map_err(|_| PreferencesError::Unavailable)
    }
}
...
/// Safe appearance-file failure category; never includes file contents.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PreferencesError {
    /// Filesystem access or atomic replacement failed.
    Unavailable,
    /// The file schema or a preference value is unsupported.
    Invalid,
}
impl std::fmt::Display for PreferencesError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            Self::Unavailable => "preferences unavailable",
            Self::Invalid => "invalid preferences",
        })
    }
}
