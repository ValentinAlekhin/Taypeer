...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
/// Current product region selected by the database workflow.
pub enum Route {
    /// Database opening and creation.
    Welcome,
    /// Unlocked database workspace.
    Workspace,
    /// Device and database settings.
    Settings,
    /// Device trust and exchange.
    Devices,
    /// Receive a database through an invitation.
    Receive,
}
...
pub enum Destination {
    ...
    Devices,
    /// Show invitation reception.
    Receive,
    /// Select a database by identity.
    Database(DatabaseId),
    /// Select a group by identity.
    Group(GroupId),
    /// Select an entry within its owning database.
    Entry(DatabaseId, EntryId),
    /// Clear the entry selection.
    ClearEntry,
    /// Create an entry in the selected group.
    NewEntry,
    /// Open database creation.
    CreateDatabase,
    /// Create or rename a group.
    GroupForm {
        /// Existing group to rename; None creates a new group.
        id: Option<GroupId>,
        /// Parent for a new group.
    ...
    CancelEdit,
    /// Show the current search projection.
    SearchResults,
    /// Restore a specific revision through the service.
    RestoreRevision(RevisionId),
    /// Clear entry history through the service.
    ClearHistory,
    /// Request application exit through all dirty-state guards.
    Quit,
}
