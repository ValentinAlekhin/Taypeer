...
impl Session {
    ...
    pub fn new(directory: &Path, worker: &Path, scenario: &'static str) -> Self {
        ...
        if !profile.join("device-ui.json").exists() {
            crate::local_settings::LocalSettings {
                relay: crate::local_settings::RelayPreference::Disabled,
                ..Default::default()
            }
            .save(&profile)
        ...
        cx.update(|cx| {
            cx.set_global(LaunchProfile(Some(profile)));
            cx.set_global(TestLaunch {
                preferences: directory.join("preferences.toml"),
