...
pub(in crate::macos::ui) fn group(
...
pub(in crate::macos::ui) fn clone_group(
    store: Entity<WorkspaceStore>,
    window: &mut Window,
    cx: &mut App,
) {
...
pub(in crate::macos::ui) fn trash_group(
    store: Entity<WorkspaceStore>,
    window: &mut Window,
    cx: &mut App,
) {
    ...
    store.update(cx, |store, _| {
        store.watch(ticket, move |store, result, window, cx| {
            let prepared = match result {
                ...
                Err(error) => {
                    store.set_notice(crate::macos::ui::workspace::error_key(&error), cx);
                    return;
                }
            };
            let Ok(operation) = taypeer_services::new_operation_id() else {
