...
impl EntryTableDelegate {
    ...
    fn sync(&mut self, cx: &App) -> TableSync {
        ...
        let refresh_columns =
            self.columns != columns || self.sort != sort || self.descending != descending;
...
fn default_column_width(column: EntryColumn) -> Pixels {
    px(match column {
...
impl TableDelegate for EntryTableDelegate {
    ...
    fn render_tr(
        ...
        div()
            .id(("entry-row", row_ix))
            .relative()
            .when_some(row.and_then(|row| row.background), |element, color| {
                element.bg(rgba(color))
            })
            .when_some(row.and_then(|row| row.foreground), |element, color| {
...
impl Entries {
    pub fn new(store: Entity<WorkspaceStore>, window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let subscriptions = vec![
            cx.observe(&store, |this, _, cx| this.sync_table(cx)),
