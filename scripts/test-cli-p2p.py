...
def response(terminal, command):
    ...
    assert not isinstance(result, dict) or "error" not in result, f"Command failed: {command.split()[0]}"
    return result
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="taypeer-public-p2p-") as directory:
        ...
        try:
            ...
            assert b'"error"' not in a.command(f'entry create --group {group} --title "PUBLIC network entry"')
            ...
            assert b'"error"' not in a.command(f'entry create --group {group} --title "PUBLIC while locked"')
